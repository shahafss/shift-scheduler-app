...
export const EmployeesListView = defineComponent({
	...
	setup(props) {
		...
		const onAddEmployeeClick = () => {
			...
			employees.value.push({name: inputValue.value, id: uuidv4(), selected: false, color: generateRandomSoftRGBA()})
		...
		function generateRandomSoftRGBA(): string {
			...
			const a = Math.random() * 0.3 + 0.5 // Alpha component (0.5-0.8)
		...
		const onDeleteEmployeeClick = () => {
			const index = employees.value.findIndex(employee => employee.id === props.selectedEmployee?.id)
		...
		return () => (
			<div>
				<div class={container}>
					...
					<div class={shadowStyle} style={{marginTop: "0.25rem", borderRadius: "0.3rem"}}>
						{showInput.value && <input class={inputStyle} onInput={({target}) => inputValue.value = (target as HTMLInputElement).value}/>}
						<button class={cx(addButtonStyle, showInput.value && inputShownButtonStyle)} onClick={() => onAddEmployeeClick()}>הוסף</button>
					</div>
				</div>
			</div>
		)
